    ...
    for filename in tqdm(os.listdir(path)):
        if filename.endswith(".dat"):
            ...
            for _, data in G.nodes.data():
                if data[key] not in node_forward:
                    node_forward[data[key]] = node_count
                    node_backward[node_count] = data[key]
                    node_count += 1

            max_edges = max(max_edges, len(G.edges()))
            min_edges = min(min_edges, len(G.edges()))
            for _, _, data in G.edges.data():
                if data[key] not in edge_forward:
                    edge_forward[data[key]] = edge_count
                    edge_backward[edge_count] = data[key]
                    edge_count += 1
...
def dfscode_to_tensor(dfscode, feature_map):
    ...
    for i, code in enumerate(dfscode):
        try:
            ...
            print(f"KeyError: '{code[2]}' not found in node_forward_dict")
...
def random_walk_with_restart_sampling(
    ...
    sampled_graph.add_node(start_node, label=G.nodes[start_node][key])
